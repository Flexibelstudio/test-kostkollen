...
const processAndFinalizeDays = async (
  userId: string,
  appData: any, // Use 'any' for the initial data structure from firestoreService
  currentAppDate: Date
) => {
    const {
        streakData: initialStreakData,
        weeklyBank: initialWeeklyBank,
        streakSaver: initialStreakSaver,
        highestStreak: initialHighestStreak,
        goals: initialGoals,
        userProfile: initialUserProfile
    } = appData;

    let lastCheckedStr = initialStreakData.lastDateStreakChecked;

    if (!lastCheckedStr) {
        const twoDaysAgo = new Date(currentAppDate);
        twoDaysAgo.setDate(currentAppDate.getDate() - 2);
        lastCheckedStr = getDateUID(twoDaysAgo);
    }

    const todayDate = new Date(currentAppDate);
    todayDate.setHours(0, 0, 0, 0);

    const [y, m, d] = lastCheckedStr.split('-').map(Number);
    let dayToProcess = new Date(y, m - 1, d);
    dayToProcess.setHours(0, 0, 0, 0);
    dayToProcess.setDate(dayToProcess.getDate() + 1);

    const datesToProcess: Date[] = [];
    while (dayToProcess.getTime() < todayDate.getTime()) {
        datesToProcess.push(new Date(dayToProcess));
        dayToProcess.setDate(dayToProcess.getDate() + 1);
    }

    if (datesToProcess.length === 0) {
        console.log("No full days missed. No processing needed.");
        return {
            ...appData,
            modalsToShow: {},
            toastToShow: null,
    ...
    let runningStreak = initialStreakData.currentStreak;
    let runningBank = { ...initialWeeklyBank };
    let runningSaver = initialStreakSaver ? { ...initialStreakSaver } : null;
    let runningHighestStreak = initialHighestStreak;
    const newSummaries: PastDaysSummaryCollection = {};
    let totalBankedInLoop = 0;

    const modalsToShow: any = {};
    ...
    for (const date of datesToProcess) {
        ...
        const minSafeCaloriesForDay = Math.max(initialGoals.calorieGoal * MIN_SAFE_CALORIE_PERCENTAGE_OF_GOAL, MIN_ABSOLUTE_CALORIES_THRESHOLD);
        const wasDaySuccessful = dailyLogForDate.length > 0 && totalNutrientsForDay.calories >= minSafeCaloriesForDay && wasCalorieGoalMetForSummary(effectiveCaloriesConsumed, initialGoals.calorieGoal, initialUserProfile.goalType);
        ...
        runningHighestStreak = Math.max(runningHighestStreak, runningStreak);

        let bankedAmountThisDay = 0;
        if (dailyLogForDate.length > 0 && totalNutrientsForDay.calories >= minSafeCaloriesForDay && totalNutrientsForDay.calories < initialGoals.calorieGoal) {
            bankedAmountThisDay = initialGoals.calorieGoal - totalNutrientsForDay.calories;
        ...
        const summaryForThisDay: PastDaySummary = {
            ...
            calorieGoal: initialGoals.calorieGoal, proteinGoalMet: totalNutrientsForDay.protein >= initialGoals.proteinGoal,
            consumedProtein: totalNutrientsForDay.protein, proteinGoal: initialGoals.proteinGoal,
            consumedCarbohydrates: totalNutrientsForDay.carbohydrates, carbohydrateGoal: initialGoals.carbohydrateGoal,
            consumedFat: totalNutrientsForDay.fat, fatGoal: initialGoals.fatGoal,
            goalType: initialUserProfile.goalType, waterGoalMet: waterLogForDate >= DEFAULT_WATER_GOAL_ML,
        ...
        if (wasDaySuccessful) {
            ...
            await addTimelineEvent(userId, streakEventData);
        }
    ...
    const currentAppDateWeekInfo = getWeekInfo(currentAppDate);
    ...
    batch.update(userDocRef, {
        ...
        highestStreak: runningHighestStreak,
    });

    await batch.commit();

    const yesterdaySummary = newSummaries[getDateUID(lastProcessedDay)];
    if (yesterdaySummary) {
        if (yesterdaySummary.goalMet) {
            modalsToShow.goalMet = { date: yesterdaySummary.date, streak: yesterdaySummary.streakForThisDay || runningStreak };
        } else {
            if (runningSaver?.available) {
                modalsToShow.streakSaver = yesterdaySummary;
            } else {
                modalsToShow.motivation = yesterdaySummary;
            }
        }
    }

    const toastToShow = totalBankedInLoop > 0 ? { message: `${totalBankedInLoop.toFixed(0)} kcal sparade till potten!`, type: "success" as const } : null;
    
    return {
        updatedStreakData: { currentStreak: runningStreak, lastDateStreakChecked: newLastCheckedDateStr },
        updatedWeeklyBank: finalBankForState,
        updatedStreakSaver: finalSaverForState,
        updatedHighestStreak: runningHighestStreak,
        updatedPastSummaries: { ...appData.pastDaySummaries, ...newSummaries },
        modalsToShow,
        toastToShow,
    };
};
  ...
  const handleFirestoreError = (error: any, operation: string) => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            if (user) {
                setCurrentUser(user);
                setIsDataLoading(true); // Keep data loading until processing is done
                setAppStatus(AppStatus.PROCESSING_DAY_END);
                try {
                    await ensureUserProfileInFirestore(user); 
                    
                    const initialAppData = await fetchInitialAppData(user.uid);
                    ...
                    if (initialAppData) {
                        const {
                            ...
                            updatedPastSummaries,
                            modalsToShow,
                            toastToShow,
                        } = await processAndFinalizeDays(user.uid, initialAppData, currentDate);

                        // Set all state based on fetched AND processed data
                        setGoals(initialAppData.goals || DEFAULT_GOALS);
                        setUserProfile(initialAppData.profile || DEFAULT_USER_PROFILE);
                        setStreakData(updatedStreakData);
                        setWeeklyBank(updatedWeeklyBank);
                        setStreakSaver(updatedStreakSaver);
                        setHighestStreak(updatedHighestStreak);
                        setHighestLevelId(initialAppData.highestLevelId || null);
                        setCommonMeals(initialAppData.commonMeals || []);
                        setWeightLogs(initialAppData.weightLogs || []);
                        setPastDaysSummary(updatedPastSummaries);
                        setUserCourseProgress(initialAppData.courseProgress || {});
                        setUnlockedAchievements(initialAppData.unlockedAchievements || {});
                        setAchievementInteractions(initialAppData.achievementInteractions || {});
                        setHasCompletedOnboarding(initialAppData.hasCompletedOnboarding || false);
                        setUserRole(initialAppData.role || 'member');
                        setUserStatus(initialAppData.status || 'pending');
                        setCurrentInterface('member');
                        setJourneyAnalysisFeedback(initialAppData.journeyAnalysisFeedback || null);
                        
                        // Show modals and toasts from processing
                        if (modalsToShow.goalMet) {
                            setShowGoalMetModalData(modalsToShow.goalMet);
                            setShowConfetti(true);
                            playAudio("levelUp");
                            setTimeout(() => setShowConfetti(false), 5000);
                        } else if (modalsToShow.streakSaver) {
                            setDayToPotentiallySave(modalsToShow.streakSaver);
                        } else if (modalsToShow.motivation) {
                            setShowMotivationModal(modalsToShow.motivation);
                        }
                        if (toastToShow) {
                            setToastNotification(toastToShow);
                            setTimeout(() => setToastNotification(null), 3500);
                            playAudio("calorieBank", 0.7);
                        }

                        // Onboarding logic
                        if ((initialAppData.role === 'member') && !initialAppData.hasCompletedOnboarding && initialAppData.status === 'approved') {
                            setShowUserProfileModal(true);
                            setIsProfileModalOnboarding(true);
                            setOnboardingStep('form');
                        }
                    } else {
                        console.error("Critical error: fetchInitialAppData returned null.");
                        resetAllLocalState();
                    }
                } catch (error: any) {
                    console.error("Error during app startup sequence:", error);
                    const isOfflineError = error.code === 'unavailable' || (error.message && (error.message.toLowerCase().includes('offline') || error.message.toLowerCase().includes('unavailable')));
                    if (isOfflineError) {
                        setToastNotification({ message: "Du är offline. Viss data kan vara inaktuell.", type: 'error' });
                    } else {
                        setToastNotification({ message: 'Ett allvarligt fel uppstod vid laddning av data.', type: 'error'});
                    }
        ...
        return () => unsubscribe();
    }, [resetAllLocalState, currentDate]);
    ...
    useEffect(() => {
        if (currentUser && !isDataLoading && userStatus === 'approved') {
            loadDataForDate(currentUser.uid, viewingDate);
        }
    }, [currentUser, viewingDate, isDataLoading, loadDataForDate, userStatus]);
    ...
    useEffect(() => {
        if (!currentUser || isDataLoading || Object.keys(pastDaysSummary).length === 0) {
            return;
        }
    ...
    }, [isDataLoading, currentUser?.uid, pastDaysSummary, currentDate]);
    ...
    }, [currentUser, isDataLoading, userStatus, loadCommunityData]);
...
useEffect(() => {
  ...
  useEffect(() => {
    const ensureValidSubscription = async () => {
        if (!currentUser || isDataLoading || !('serviceWorker' in navigator) || !('PushManager' in window)) {
            return;
        }
    ...
    useEffect(() => {
        if (!checklistState || !currentUser || isDataLoading) return;
    ...
    }, [checklistState, currentUser, isDataLoading, userRole, userStatus]);

    useEffect(() => {
        if (!currentUser || isDataLoading || !hasCompletedOnboarding) {
          setChecklistState(null);
          return;
        }
    ...
    }, [isDataLoading, hasCompletedOnboarding, currentUser]);
  ...
  useEffect(() => {
    if (isDataLoading || !currentUser || !userProfile.isCourseActive) {
        return;
    }
  ...
  }, [isDataLoading, currentUser, userProfile.isCourseActive, userCourseProgress, streakData.currentStreak, unlockLesson]);
  ...
  useEffect(() => {
    if (highestStreak > 0 && !isDataLoading) {
  ...
  }, [highestStreak, isDataLoading, handleUnlockAchievement]);
