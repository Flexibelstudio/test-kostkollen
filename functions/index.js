...
async function sendNotificationToUser(userId, payload, notificationType) {
  ...
  if (
    ...
    userData.notificationSettings[notificationType] === false
  ) {
    logger.log(
      `Skipping notification type '${notificationType}' for user ${userId} due to user settings.`,
    );
    return;
  }
    ...
    webpush
      ...
      .catch((error) => {
        if (error.statusCode === 404 || error.statusCode === 410) {
          logger.warn(
            `Removing invalid subscription for user ${userId} (Status: ${error.statusCode})`,
          );
          dirty = true;
        } else {
...
exports.onTimelineEventCreated = functions.firestore
  ...
  .onCreate(async (snapshot) => {
    ...
    const isSystemOrCoach =
      event.userId === "system" ||
      event.isGlobal ||
      (event.visibleTo && event.visibleTo.includes("GLOBAL"));
    ...
    if (isSystemOrCoach) {
      ...
      if (
        ...
        participantsSnapshot.forEach((doc) => {
          ...
          if (
            data.userId &&
            (data.status === "fas1" || data.status === "fas2")
          ) {
  ...
  .onUpdate(async (change, context) => {
    try {
      ...
      if (newStreak > oldStreak) {
        ...
        if (
          after.measurementMethod === "scale" &&
          after.desiredWeightChangeKg
        ) {
          goalTextAtPost = `Mål: ${after.desiredWeightChangeKg > 0 ? "+" : ""}${after.desiredWeightChangeKg} kg`;
        } else if (after.measurementMethod === "inbody") {
          if (after.desiredFatMassChangeKg)
            goalTextAtPost = `Mål: ${after.desiredFatMassChangeKg} kg fett`;
          else if (after.desiredMuscleMassChangeKg)
            goalTextAtPost = `Mål: +${after.desiredMuscleMassChangeKg} kg muskler`;
        }

        let goalSummary = "Ej satt";
        if (after.goalType === "maintain") goalSummary = "Bibehålla";
        else if (after.goalType === "lose_fat")
          goalSummary = `${after.desiredFatMassChangeKg || after.desiredWeightChangeKg || ""} kg fett`;
        else if (after.goalType === "gain_muscle")
          goalSummary = `${after.desiredMuscleMassChangeKg || after.desiredWeightChangeKg || ""} kg muskler`;
        ...
        try {
          ...
          if (!latestLogSnap.empty) {
            ...
            currentMuscleMass =
              latestLog.skeletalMuscleMassKg ?? currentMuscleMass;
          }
        } catch (e) {
          logger.warn(
            "Could not fetch weight logs for progress calculation",
            e,
          );
        }
        ...
        const isFatLossGoal =
          !isScaleGoal &&
          after.desiredFatMassChangeKg &&
          after.desiredFatMassChangeKg < 0;
        const isMuscleGainGoal =
          !isScaleGoal &&
          after.desiredMuscleMassChangeKg &&
          after.desiredMuscleMassChangeKg > 0;
        ...
        if (start != null && current != null && goalChange) {
          ...
          let changeAchieved =
            goalChange > 0 ? current - start : start - current;
        ...
        try {
          const bootcampSnap = await db
            .collectionGroup("participants")
            .where("userId", "==", userId)
            .get();
          if (!bootcampSnap.empty) {
            // Find the active one
            const activeParticipant = bootcampSnap.docs
              .map((d) => d.data())
...
exports.onReactionAdded = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    for (const emoji in afterReactions) {
      ...
      const newUids = Object.keys(usersAfter).filter(
        (uid) => !usersBefore[uid],
      );
  ...
  .onRun(async (context) => {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      if (localHour === 10 && user.lastLogDate) {
        ...
        if (
          diffDays === 3 &&
          user.lastInactivityReminderSent !== todayDateString
        ) {
          const payload = {
            notification: {
              title: "Vi saknar dig! 🥺",
          ...
          await db
            .collection("users")
            .doc(userId)
            .update({ lastInactivityReminderSent: todayDateString });
        }
      }
      ...
      if (localHour === 19 && user.currentStreak > 0) {
        ...
        if (MILESTONE_STREAKS.includes(nextDayStreak)) {
          if (
            ...
            user.lastMilestoneNudgeSentFor !== todayDateString
          ) {
            const payload = {
              notification: {
                title: "Du är så nära! 🔥",
            ...
            await db
              .collection("users")
              .doc(userId)
              .update({ lastMilestoneNudgeSentFor: todayDateString });
          }
        }
      }
      ...
      const preferredDay = isBootcampActive
        ? "söndag"
        : (user.preferredWeighInDay || "måndag").toLowerCase();
      ...
      if (
        ...
        user.lastTrialReminderSent !== todayDateString
      ) {
        try {
          ...
          if (diffDays >= 1 && diffDays <= 3 && user.status !== "archived") {
            ...
            await db
              .collection("users")
              .doc(userId)
              .update({ lastTrialReminderSent: todayDateString });
          }
        } catch (trialErr) {
          logger.error(
            "Error processing trial reminder in scheduler for user " + userId,
            trialErr,
          );
        }
      }
...
const MIN_SAFE_CALORIE_PERCENTAGE_OF_GOAL = 0.8;
...
exports.manualSummarizeYesterday = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (
      !callerDoc.exists ||
      (callerDoc.data().role !== "admin" && callerDoc.data().role !== "coach")
    ) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "User must be an admin or coach.",
      );
    }

    const serverTime = new Date();
    const yesterday = new Date(
      serverTime.toLocaleString("en-US", { timeZone: "Europe/Stockholm" }),
    );
    ...
    if (yesterdayDateUID >= todayDateUID) {
      logger.warn(
        `Attempted to summarize a future/current date (${yesterdayDateUID}). Aborting.`,
      );
      return {
        success: false,
        message: `Attempted to summarize a future/current date (${yesterdayDateUID}). Aborting.`,
      };
    }
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      if (
        user.lastDateStreakChecked &&
        user.lastDateStreakChecked >= yesterdayDateUID
      )
        continue;
      if (!user.goals || typeof user.goals.calorieGoal !== "number") continue;

      try {
        const mealLogsSnap = await db
          .collection("users")
          .doc(userId)
          .collection("mealLogs")
          .where("dateString", "==", yesterdayDateUID)
          .get();
        const dailyLogForDate = mealLogsSnap.docs.map((d) => d.data());

        const totalNutrients = dailyLogForDate.reduce(
          (acc, meal) => {
        ...
        const minSafeCalories =
          user.goals.calorieGoal * MIN_SAFE_CALORIE_PERCENTAGE_OF_GOAL;
        const isYesterdayMonday = yesterday.getDay() === 1;
        const bankedCalories = isYesterdayMonday
          ? 0
          : user.weeklyBank?.bankedCalories || 0;
        ...
        if (user.goalType === "lose_fat" || user.goalType === "maintain") {
          if (
            totalNutrients.calories >= minSafeCalories &&
            totalNutrients.calories < user.goals.calorieGoal
          ) {
            bankedAmountThisDay =
              user.goals.calorieGoal - totalNutrients.calories;
        ...
        allWriteOps.push({
          ...
          data: {
            ...
            "weeklyBank.bankedCalories": admin.firestore.FieldValue.increment(
              bankedAmountThisDay - usedFromBank,
            ),
          },
          type: "update",
        });
...
exports.createCheckoutSession = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!priceId) {
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Kunde inte hitta ett pris för produkten.",
      );
    }
    ...
    try {
      ...
      if (existingCustomers.data.length > 0) {
        ...
        console.log(
          `Hittade befintlig Stripe-kund för ${userEmail}: ${customerId}`,
        );
      } else {
...
exports.cancelSubscription = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      "aborted",
      error.message || "Kunde inte avsluta prenumerationen.",
    );
  }
});
...
exports.undoCancelSubscription = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "aborted",
        error.message || "Kunde inte ångra uppsägningen.",
      );
    }
  },
);
      ...
      notification: {
        title:
          chatData.type === "direct"
            ? message.senderName
            : `${message.senderName} i ${chatData.name}`,
        body:
          message.text ||
          (message.imageUrl ? "Skickade en bild" : "Nytt meddelande"),
        icon: "/icons/icon-192x192.png",
        badge: "/icons/badge-96x96.png",
...
exports.stripeWebhook = functions.https.onRequest(async (req, res) => {
  ...
  const endpointSecret =
    ...
    getSafeConfig("stripe", "webhook_secret");
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      if (firebaseUid) {
        if (cohortId) {
          ...
          if (session.subscription) {
            try {
              ...
              logger.error(
                "Error retrieving subscription from Stripe in checkout.session.completed:",
                subErr,
              );
            }
          }
        ...
        logger.warn(
          "No firebaseUid in session metadata for checkout.session.completed",
          { sessionId: session.id },
        );
      }
    } else if (event.type === "customer.subscription.updated") {
      const subscription = event.data.object;
      // Bestäm status baserat på Stripe-fälten
      let targetStatus = "active";
      if (subscription.status === "trialing") {
        targetStatus = "trialing";
      } else if (subscription.cancel_at_period_end) {
        targetStatus = "canceling";
      }
        ...
        if (!usersSnapshot.empty) {
          ...
          usersSnapshot.forEach((doc) => {
            ...
            if (currentPeriodEndIso) {
              updateData.currentPeriodEnd = currentPeriodEndIso;
            } else {
      ...
      if (!usersSnapshot.empty) {
        const promises = usersSnapshot.docs.map(async (doc) => {
          ...
          if (
            ...
            userData.subscriptionStatus !== "canceling"
          ) {
            const payload = {
              notification: {
                title: "Din gratisvecka tar snart slut ⚡",
            ...
            await doc.ref.update({
              lastTrialReminderSent: stockholmNow().dateString,
            });
          }
        });
        await Promise.all(promises);
...
exports.publishScheduledPosts = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      const bootcampsSnapshot = await db
        .collection("bootcampCohorts")
      ...
      if (!activeBootcamps.find((b) => b.id === "solo_group")) {
        ...
        if (
          !soloGroupData.startDate &&
          soloDoc.exists &&
          soloDoc.data().startDate
        ) {
      ...
      for (const postDoc of scheduledPostsSnapshot.docs) {
        ...
        if (groupId === "all") {
          ...
          targetBootcamps = activeBootcamps.filter(
            (b) => b.id === "solo" || b.id === "solo_group",
          );
        } else {
        ...
        for (const bootcamp of targetBootcamps) {
          ...
          if (typeof bootcamp.startDate === "string") {
            ...
            bootcamp.startDate &&
            typeof bootcamp.startDate.toDate === "function"
          ) {
          ...
          if (shouldPublishToday) {
            ...
            if (
              ...
              postData.excludedGroups.includes(bootcamp.id)
            ) {
              continue;
            }
      ...
      if (publishedCount > 0) {
        ...
        logger.log(
          "No scheduled posts matched the current date/time for active bootcamps.",
        );
      }
...
exports.geminiApiProxy = functions
  .runWith({
    ...
    cors(req, res, async () => {
      ...
      if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return res
          ...
          .send({
            error: "Unauthorized",
            message: "Logga in för att använda AI.",
          });
      }
      ...
      try {
        ...
        return res
          .status(403)
          .send({ error: "Forbidden", message: "Ogiltig session." });
      }
        ...
        if (error.message === "RATE_LIMIT_EXCEEDED") {
          return res
            ...
            .send({
              ...
              message: "Du har nått timgränsen för AI-frågor.",
            });
        }
      ...
      try {
        ...
        if (req.headers["content-type"])
          headersToForward.set("Content-Type", req.headers["content-type"]);
