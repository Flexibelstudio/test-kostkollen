    ...
    return (
        <>
            <form onSubmit={handleSave} className="bg-white p-4 sm:p-5 rounded-xl shadow-soft-lg border border-neutral-light">
                ...
                {isEditing ? (
                    <div className="space-y-6 animate-fade-in pt-6 border-t border-neutral-light/70">
                        <section aria-labelledby="current-measurements-heading">
                            <h4 id="current-measurements-heading" className="text-lg font-semibold text-neutral-dark mb-3">Aktuell mätning</h4>
                            <div className="space-y-4">
                                ...
                                <div className="pt-3 border-t border-neutral-light/60">
                                    <p className="text-sm text-neutral-dark mb-3 flex items-start">
                                        ...
                                        <span>Har du gjort en InBody-mätning? Fyll i valfria fält nedan. Dessa blir startpunkten för ditt nya mål.</span>
                                    </p>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4 mt-2">
                                        <div>
                                            <label htmlFor="skeletalMuscleMassKg-editor" className="block text-base font-medium text-neutral-dark">Muskelmassa (kg)</label>
                                            <input type="number" name="skeletalMuscleMassKg" id="skeletalMuscleMassKg-editor" value={profile.skeletalMuscleMassKg == null ? '' : profile.skeletalMuscleMassKg} onChange={handleProfileChange} className={inputClass} min="0" step="0.1" placeholder="Valfritt" />
                                        </div>
                                        <div>
                                            <label htmlFor="bodyFatMassKg-editor" className="block text-base font-medium text-neutral-dark">Fettmassa (kg)</label>
                                            <input type="number" name="bodyFatMassKg" id="bodyFatMassKg-editor" value={profile.bodyFatMassKg == null ? '' : profile.bodyFatMassKg} onChange={handleProfileChange} className={inputClass} min="0" step="0.1" placeholder="Valfritt" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <section aria-labelledby="activity-level-heading-editor">
                            <h4 id="activity-level-heading-editor" className="text-lg font-semibold text-neutral-dark mb-3">Aktivitetsnivå</h4>
                             <div className="grid grid-cols-1 gap-3">
                                {activityLevelOptions.map(opt => (
                                    ...
                                    >
                                        <div className="flex items-center"><span className="text-3xl mr-4">{opt.emoji}</span><div><p className={`font-semibold ${profile.activityLevel === opt.value ? 'text-primary-darker' : 'text-neutral-dark'}`}>{opt.label}</p><p className="text-sm text-neutral-dark">{opt.description}</p><p className="text-xs text-neutral mt-1">{opt.example}</p></div></div>
                                    </button>
                                ))}
                            </div>
                        </section>
                        ...
                        <section aria-labelledby="measurement-method-heading-editor">
                            <h4 id="measurement-method-heading-editor" className="text-lg font-semibold text-neutral-dark mb-2">Hur sätter du ditt mål?</h4>
                        ...
                        <div className="mt-8 pt-6 border-t border-neutral-light/70 flex flex-col sm:flex-row justify-end items-center gap-4">
                             <button type="button" onClick={handleCancel} className="w-full sm:w-auto px-6 py-3 border border-neutral-light rounded-lg shadow-sm text-lg font-semibold text-neutral-dark bg-white hover:bg-neutral-light focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary active:scale-95 transform interactive-transition">Avbryt</button>
            ...
            {showResetConfirmModal && (
                 <div className="fixed inset-0 bg-neutral-dark bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4 z-[100] animate-fade-in" onClick={() => setShowResetConfirmModal(false)} role="dialog" aria-modal="true" aria-labelledby="confirm-goal-reset-title">
                    <div className="bg-white p-6 rounded-lg shadow-soft-xl w-full max-w-sm animate-scale-in" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-start">
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div className="mt-2"><p className="text-sm text-neutral-dark">Om du uppdaterar ditt mål kommer dina nuvarande framsteg och tidslinje att nollställas. Ditt gamla mål kommer att raderas.</p></div>
                            </div>
                        </div>
                        <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-3">
                            <button type="button" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:w-auto sm:text-sm" onClick={confirmGoalReset}>Ja, uppdatera</button>
                            <button type="button" className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:w-auto sm:text-sm" onClick={() => setShowResetConfirmModal(false)}>Avbryt</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showBmrTdeeInfoModal && (
                <div className="fixed inset-0 bg-neutral-dark bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-[60] p-4 animate-fade-in" onClick={() => setShowBmrTdeeInfoModal(false)}>
                    <div onClick={e => e.stopPropagation()}><BmrTdeeInfoModal onClose={() => setShowBmrTdeeInfoModal(false)} /></div>
                </div>
            )}
        </>
    );
