    ...
    <div className={`p-5 bg-white shadow-soft-lg rounded-xl border border-neutral-light interactive-transition ${disabled ? 'opacity-60' : 'hover:shadow-soft-xl'}`}>
      ...
      <div className="mb-4">
        ...
        <div className="w-full bg-neutral-light rounded-full h-6 shadow-inner overflow-hidden">
          <div
            className="bg-primary h-full rounded-full transition-all duration-500 ease-out" // Changed from bg-secondary to bg-primary
      ...
      <div className="flex flex-wrap gap-2.5 justify-center">
        {logAmounts.map((amount) => (
          <button
            ...
            className={`${buttonBaseClass} bg-primary hover:bg-primary-darker text-white focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
       ...
       {currentWaterMl >= waterGoalMl && waterGoalMl > 0 && (
        <p className="text-sm text-green-600 mt-3 text-center font-medium animate-fade-in">
