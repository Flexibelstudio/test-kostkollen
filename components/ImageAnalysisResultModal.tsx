...
const ImageAnalysisResultModal: React.FC<ImageAnalysisResultModalProps> = ({ show, analysisResult, imageDataUrl, onLog, onClose, defaultMealType = null }) => {
  ...
  const [editedCalories, setEditedCalories] = useState<string>('0');
  const [editedProtein, setEditedProtein] = useState<string>('0');
  const [editedCarbohydrates, setEditedCarbohydrates] = useState<string>('0');
  const [editedFat, setEditedFat] = useState<string>('0');
