...
import { ProteinIcon, FireIcon as FatIcon } from './icons';

// Custom icon for Weight to avoid conflicts
         ...
         <div className="bg-white p-4 sm:p-6 rounded-xl shadow-soft-lg border border-neutral-light">
            ...
            <div className="flex justify-center flex-wrap gap-2 mb-4">
                ...
                {hasMuscleData && (
                    <LegendItem 
                        label="Muskler" 
                        icon={<ProteinIcon className="w-4 h-4" />} 
                ...
                {hasFatData && (
                    <LegendItem 
                        label="Fett" 
                        icon={<FatIcon className="w-4 h-4" />} 
                    ...
                    <div className="relative w-full overflow-x-auto custom-scrollbar">
                        <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} role="img" aria-labelledby="chartTitle">
                            ...
                            <g transform={`translate(${margin.left},${margin.top})`}>
                                ...
                                {/* Lines */}
                                {seriesVisibility.fat && hasFatData && <path d={createPath('fatY')} fill="none" stroke="#F59E0B" strokeWidth="2.5" strokeLinecap="round" opacity="0.8" />}
                                {seriesVisibility.muscle && hasMuscleData && <path d={createPath('muscleY')} fill="none" stroke="#f97316" strokeWidth="2.5" strokeLinecap="round" opacity="0.8" />}
                                {seriesVisibility.weight && <path d={createPath('weightY')} fill="none" stroke="#51a1a1" strokeWidth="3.5" strokeLinecap="round" />}
                                ...
                                {points.map((p, i) => (
                                    <g key={`point-group-${i}`} onMouseEnter={() => handleMouseEnter(p)} onMouseLeave={() => setTooltip(null)} className="cursor-pointer">
                                        ...
                                        {seriesVisibility.weight && <circle cx={p.x} cy={p.weightY} r="5" fill="#51a1a1" stroke="white" strokeWidth="2" />}
                                        {seriesVisibility.muscle && p.muscleY !== null && <circle cx={p.x} cy={p.muscleY} r="5" fill="#f97316" stroke="white" strokeWidth="2" />}
                                        {seriesVisibility.fat && p.fatY !== null && <circle cx={p.x} cy={p.fatY} r="5" fill="#F59E0B" stroke="white" strokeWidth="2" />}
                                    </g>
                                ))}
