...
const AICoachModal: React.FC<AICoachModalProps> = ({
  ...
  const getCoachVisuals = (style: CoachStyle) => {
    if (style === "soft")
      return {
        emoji: COACH_PERSONAS.soft.emoji,
        colorClass: "bg-green-100 text-green-600",
      };
    if (style === "hard")
      return {
        emoji: COACH_PERSONAS.hard.emoji,
        colorClass: "bg-red-100 text-red-600",
      };
    return {
      emoji: COACH_PERSONAS.balanced.emoji,
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === botMessagePlaceholder.id
            ? {
                ...m,
                text: `Ursäkta, ett fel inträffade: ${errorMessage}`,
                isStreaming: false,
              }
            : m,
        ),
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-neutral-light/70 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2
                id="ai-coach-modal-title"
                className="text-xl font-semibold text-neutral-dark leading-tight"
              >
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                {msg.sender === "bot" && (
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mb-1 ${colorClass} bg-opacity-50`}
                  >
                    {persona.imageUrl ? (
                      <img
                        src={persona.imageUrl}
                ...
                <div
                  className={`max-w-xs sm:max-w-md p-3 rounded-2xl ${msg.sender === "user" ? "bg-primary text-white rounded-br-lg" : "bg-neutral-light text-neutral-dark rounded-bl-lg"}`}
                >
                  <div
                  ...
                  />
                  {msg.isStreaming && !msg.chartData && msg.text.length > 0 && (
                    <div className="inline-block w-1.5 h-1.5 bg-neutral-dark rounded-full animate-ping ml-1"></div>
                  )}
                </div>
              </div>
            );
          ...
          {isLoading &&
            messages.some(
              (m) =>
                m.sender === "bot" && m.isStreaming && !m.text && !m.chartData,
            ) && (
              <div className="flex items-center gap-3 justify-start p-2 pl-1 animate-fade-in">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${colorClass} animate-pulse`}
                >
                  {persona.imageUrl ? (
                    <img
                      src={persona.imageUrl}
        ...
        <footer className="p-4 border-t border-neutral-light/70 flex-shrink-0 bg-white">
          ...
          >
            ...
            <button
              ...
              className="p-3 bg-primary text-white rounded-full shadow-sm hover:bg-primary-darker active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed interactive-transition"
            >
              <svg
